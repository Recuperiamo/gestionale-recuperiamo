...
function AdminDashboard() {
  ...
  // Questo blocco di codice è stabile e non verrà modificato.
  // Calcola le ore svolte in tempo reale per la visualizzazione.
  ...
  const handleAddPackage = async (e, clientId) => { e.preventDefault(); const clientToUpdate = clients.find(c => c.id === clientId); if (!clientToUpdate) return; const newPackage = { id: Date.now(), name: newPackageDetails.name, totalHours: parseFloat(newPackageDetails.totalHours), bookings: [] }; const newPackages = [...(clientToUpdate.packages || []), newPackage]; await updateDoc(doc(db, "clients", clientId), { packages: newPackages }); setPackageFormClientId(null); setNewPackageDetails({ name: '', totalHours: '' }); };
  const handleUpdatePackage = async (e, clientId, packageId) => { e.preventDefault(); const clientToUpdate = clients.find(c => c.id === clientId); if (!clientToUpdate) return; const newPackages = clientToUpdate.packages.map(pkg => { if (pkg.id === packageId) { return { ...pkg, ...editingPackage }; } return pkg; }); await updateDoc(doc(db, "clients", clientId), { packages: newPackages }); setEditingPackage(null); };
  ...
  const handleAddBooking = async (e, clientId, packageId) => { e.preventDefault(); const clientToUpdate = clients.find(c => c.id === clientId); if (!clientToUpdate) return; const pkg = clientToUpdate.packages.find(p => p.id === packageId); if (!pkg) return; let newBooking; const hours = parseFloat(bookingDetails.hours); const startDate = `${bookingDetails.date}T${bookingDetails.time}`; if (bookingType === 'single') { newBooking = { id: Date.now(), type: 'single', dateTime: startDate, hoursBooked: hours, requests: {} }; } else { if (bookingDetails.days.length === 0) return alert('Seleziona un giorno.'); newBooking = { id: Date.now(), type: 'recurring', startDate, hoursBooked: hours, recurrence: { weeks: parseInt(bookingDetails.weeks, 10), days: bookingDetails.days }, cancelledDates: [], requests: {} }; } await updateDoc(doc(db, "clients", clientId), { packages: clientToUpdate.packages.map(p => p.id === packageId ? { ...p, bookings: [...(p.bookings || []), newBooking] } : p) }); setBookingForm(null); };
  ...
  const handleApproveReschedule = async () => {
    ...
    if (bookingToUpdate.requests) {
        bookingToUpdate.requests[dateString] = {
            ...bookingToUpdate.requests[dateString],
            resolved: true, status: `Approvata e Spostata al ${new Date(newDateTime).toLocaleDateString()}`, notified: false, notification: { message: notificationMessage }
        };
    }
