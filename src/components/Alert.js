...
export default function Alert({ message, type = "error", onClose }) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={onClose}
        className="ml-2 flex items-center justify-center bg-white bg-opacity-30 hover:bg-opacity-60 rounded-full w-5 h-5 transition duration-150"
        aria-label="Chiudi"
        title="Chiudi"
        tabIndex={0}
      >
        <svg className="w-3 h-3 text-gray-600" viewBox="0 0 20 20" stroke="currentColor" fill="none">
