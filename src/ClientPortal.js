...
import 'react-calendar/dist/Calendar.css'; // Stile base del calendario
...
function ClientPortal({ user }) {
                ...
                if(booking.requests) {
                    Object.entries(booking.requests).forEach(([dateString, request]) => {
                        if(request.resolved && !request.notified) {
                            newNotifications.push(`La tua richiesta per la lezione del ${new Date(dateString).toLocaleDateString()} è stata valutata: ${request.status}`);
  ...
  (clientData.packages || []).forEach(pkg => {
    (pkg.bookings || []).forEach(booking => {
      const baseInfo = { packageName: pkg.name, hoursBooked: booking.hoursBooked, isProcessed: booking.isProcessed };
      if (booking.type === 'single') {
        const dateString = new Date(booking.dateTime).toISOString().split('T')[0];
        const request = (booking.requests || {})[dateString];
        allOccurrences.push({ ...baseInfo, effectiveDate: new Date(booking.dateTime), status: request?.status || (booking.isProcessed ? 'Svolta' : 'Da Svolgere') });
      } else {
        ...
        for (let i = 0; i < booking.recurrence.weeks; i++) {
          (booking.recurrence.days || []).forEach(day => {
            ...
            const request = (booking.requests || {})[dateString];
            if (!isCancelled) {
              allOccurrences.push({ ...baseInfo, effectiveDate: d, isProcessed, status: request?.status || (isProcessed ? 'Svolta' : 'Da Svolgere') });
            }
          });
        }
      }
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-slate-200">
        <header className="text-center py-16 sm:py-20">
            ...
            <div className="flex justify-between items-center">
                ...
                <button className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onClick={() => signOut(auth)}>Esci</button>
            </div>
        </header>
            ...
            <div className="bg-white text-slate-800 p-8 rounded-2xl shadow-lg">
                ...
                {(clientData.packages && clientData.packages.length > 0) ? (
                    clientData.packages.map(pkg => {
                        const visibleOccurrences = allOccurrences.filter(occ => occ.packageName === pkg.name && !occ.isCancelled);
                        const totalCompletedHours = visibleOccurrences.filter(occ => occ.isProcessed).reduce((sum, occ) => sum + occ.hoursBooked, 0);
                        return (
                            <div key={pkg.id} className="mb-6 pb-6 border-b border-slate-200">
                                ...
                                <div className="mt-2 space-y-2 text-lg">
                                    ...
                                    <p className="font-bold"><strong>Ore Rimanenti:</strong> {pkg.remainingHours}h</p>
                                </div>
                            </div>
                        );
                    })
